...
gulp.task('sprite', function () {
  return gulp.src('./src/icon/*.png')//需要合并的图片地址
    .pipe(spritesmith({
      ...
      cssVarMap: function (sprite) {
        ...
        if (/_normal/img.test(name)) {
          sprite.name = sprite.name.replace('_normal', '')
        } else if (/_hover/img.test(name)) {
          sprite.name = sprite.name.replace('_hover', ':hover')
        }
      }
    }))
    .pipe(gulp.dest('./src'))
...
/*let apiProxy = proxy('/api', {
  target: 'https://test-news.qaqgame.com/',
  changeOrigin: true,
  logLevel: 'debug'
})*/
